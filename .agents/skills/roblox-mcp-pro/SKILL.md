...
- **Property value shapes.** Vectors are `[x,y,z]`; Color3 is `[r,g,b]` (0–1); UDim2 is
  `[[xScale,xOffset],[yScale,yOffset]]`; enums and BrickColor are their string names. Values are
  coerced to the property's current type, so match the existing type.
...
- `action: "start"` — snapshot + begin two-way watching. Defaults to the script-bearing services
  ...
  `roots: ["Workspace.Systems", …]` to scope differently. Editing a synced `.luau` file updates
  the script in Studio; editing the script in Studio updates the file.
